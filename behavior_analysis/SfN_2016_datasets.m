...
% L_sst
% L_lst
o = datasets_eye_hand(current_dataset);
...
GLO.append_pdfs                     =   0;
GLO.parent_folder                   =   '';
GLO.folder_to_save                  =   ['K:\Projects\Pulv_eye_hand\behavior\SfN2016' filesep o.dataset filesep date];
...
group{1}                        = o.mon;
dates_subject_in{1}             = o.ses;
batching{1}.runs                = o.run;  % either empty or specific runs specified
batching_type{1}                = 1; % 1 run by run, 2 session by session, 3 group by group
batching{1}.range_of_dates      = 0;

subject_ID{2}='Experimental';
group{2}                        = o.mon;
dates_subject_in{2}             = o.ses;
batching{2}.runs                = o.run;  % either empty or specific runs specified
