...
GLO.append_pdfs                     =   0;
GLO.parent_folder                   =   '';
GLO.folder_to_save                  =   'Y:\Projects\Pulv_oculomotor\behavior\monkeys_compared';
...
group{1}                        = repmat({'Linus_phys'},size(filelist_formatted.Lin_dPulv_Msac_opt,1),1);
dates_subject_in{1}             = filelist_formatted.Lin_dPulv_Msac_opt(:,1);
batching{1}.runs                = filelist_formatted.Lin_dPulv_Msac_opt(:,2);  % either empty or specific runs specified
batching_type{1}                = 2; % 1 run by run, 2 session by session, 3 group by group
batching{1}.range_of_dates      = 0;

subject_ID{2}='Experimental';
group{2}                        = repmat({'Curius_phys'},size(filelist_formatted.Cur_dPulv_Msac_opt,1),1);
dates_subject_in{2}             = filelist_formatted.Cur_dPulv_Msac_opt(:,1);
batching{2}.runs                = filelist_formatted.Cur_dPulv_Msac_opt(:,2);  % either empty or specific runs specified
