...
keys.condition_parameters={'fix_index'};
...
% keys.pop(cc).normalization           = 'per_position';
...
% keys.pop(cc).condition_parameters={'reach_hand','choice','perturbation'};
...
keys.pop(cc).FR_subtract_baseline    = 0;
keys.pop(cc).ylim                    = [];
keys.pop(cc).plot_per_position       = 1; %% set back to 0 afterwards
keys.pop(cc).plot_RF                 = 1;
keys.pop(cc).tt.choices              = 0;
keys.pop(cc).tt.hands                = 0;
...
keys.clf(cc).position_and_plotting_arrangements={'fixation','movement vectors'};
keys.clf(cc).tt.selection={'target','dPul'};
keys.clf(cc).tt.hands=0;
keys.clf(cc).tt.choices=0;
keys.gfl(cc).tt.perturbations=0;
keys.gfl(cc).split_SUs={'Cur_20151204_22','Cur_20151204_24'};
