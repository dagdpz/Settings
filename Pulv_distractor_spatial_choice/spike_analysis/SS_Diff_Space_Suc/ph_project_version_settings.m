...
%keys.task_types             ={'mem'};
%keys.datasets               ={'Msac'};
keys.position_and_plotting_arrangements         ={ 'SpS_Diff_SS', 'SpT_Diff_SpaceTar'}; %, 'Sgl_Diff_Pos_Suc_SaccadeEpoch'
...
keys.cal.MA_selection                   ={'display',0,'keep_raw_data',1,'saccade_definition',4,'reach_1st_pos',1,'success',1,'n_targets', 2,'difficulty_colors', [120 22 0; 60 60 0; 60 60 60], 'correlation_conditions',{}};    
...
% !! n_targets displays two stimuli !!
...
keys.pop(cc).tt.unselect                ={'ch_Tacq_spaceLR_Vsac_SST', '-' }; 
...
keys.pop(cc).tt.tasktypes               = {'Vsac'};
keys.pop(cc).group_parameter            = 'ungrouped'; % grouping units - hand_tuning from the Tuning-table
%keys.pop(cc).group_parameter            = 'ch_Tacq_SglTar_Space_Vsac_SpT'; % ch_Tacq_spaceLR_Vsac_Stigrouping units - hand_tuning from the Tuning-table
keys.pop(cc).group_excluded             = {''};%{'susu','ensu','suen','-su','su-','--'};
keys.pop(cc).conditions_to_plot         = {'Vsac'}; 
keys.pop(cc).epoch_PF                   = 'Tacq';               % epoch in which preference defines target location for "pref" plots
keys.pop(cc).epoch_RF                   = 'Tacq';               % epoch for which gaussian response fields will be plotted (if plot_RF ~ 0)
...
keys.pop(cc).tt.tasktypes               ={'Vsac'}; 
%keys.pop(cc).group_parameter            = 'ungrouped'; % grouping units - hand_tuning from the Tuning-table
keys.pop(cc).group_parameter            = 'ch_Tacq_Tar_Dis_Vsac_SpT'; % ch_Tacq_spaceLR_Vsac_Stigrouping units - hand_tuning from the Tuning-table
...
keys.colors.TD_SU_IS =  [0 153 153]; 
keys.colors.TD_SU_CS = [255 0 255]; 
keys.colors.TD_SU_NP = [128 128 128]; 
keys.colors.TD_SU_PF = [128 128 128];
%% 
