...
ecg_bna_cfg = [];
ecg_bna_cfg.LFP_version='ver_Bacchus_reproduced_by_Luba';
ecg_bna_cfg.spikes_version='ECG_TaskRest_Bacchus_reproduced_by_Luba';
...
sessions{1}= ...
    sort([20211116, 20211117, 20211207, 20211208, 20211214, 20211222, 20220105, ...
    20220106, 20220203, 20220211, 20220221, 20220222, ...
    20220224, 20220225, 20220309, 20220310, 20220315, 20220318, 20220322]); % skipped 20211005, skip 20210829 (no ECG), 20220125, 20220126, 

% skipped 20210805, 
% finished for 20210715, 20210716, 20210720, 20210722, 20210723, 20210729, ...
%     20210730, 20210803, 20210806, 20210826, 20210827, 20210903, ...
%     20210905, 20210906, 20210930, 20211001, 20211007, ...
%     20211013, 20211014, 20211019, 20211027, 20211028, 20211102, 20211103, ...
...
ecg_bna_cfg.analyses = {'Rpeak_evoked_spike_histogram'}; % , 'Rpeak_evoked_LFP', 'Rpeak_evoked_ECG', 'Rpeak_evoked_TFS', 
...
ecg_bna_cfg.compare.targets = {'dPul_R', 'dPul_L','VPL_R', 'VPL_L','mdT_L','mdT_R'}; %{'VPL_R'};
...
ecg_bna_cfg.significance_window=[-0.25 0.25]; % Ueda et al., 2019, Frontiers in Veterinary Science: duration of QT interval corresponding to ventricular systole = 226
...
ecg_bna_cfg.unit_exclusion.SNR_thresholds             = [4 26]; % above 26 are likely units with 1-2 spikes and therefore having high SNRs
