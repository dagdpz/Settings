...
ecg_bna_cfg.LFP_version='ECG_TaskRest_Bacchus_merged';
ecg_bna_cfg.spikes_version='ECG_TaskRest_Bacchus_merged';
...
% used for analysis
ecg_bna_cfg.use_datasets = [5 6 7 8];
...
ephys_version='ECG_TaskRest_Bacchus_merged';
ephys_folder=['Y:\Projects\' project '\ephys\' ephys_version filesep];
wc_folder = ['Y:\Data\Sortcodes\' ecg_bna_cfg.monkey '_phys\'];
...
sessions{1} = [20210722, 20210723, 20210729, 20210730, 20210805, 20210806, ...
    20210826, 20210827, 20210903, 20210905, 20210906, 20210930, 20211001, ...
    20211005, 20211007, 20211012, 20211013, 20211014, 20211019, 20211027, ...
    20211028, 20211102, 20211103, 20211116, 20211117, 20211207, 20211214, ...
    20211222, 20220105, 20220106, 20220203, 20220211, 20220221, 20220222, ...
    20220224, 20220225, 20220309, 20220310, 20220315, 20220318, 20220322]; % finished for 20210715, 20210716, 20210720, 
...
for m=1:numel(monkeys)
    ...
    for s=1:numel(sessions{m})
        ...
        ecg_bna_cfg.session_info(cumulative_sessions) = ...
            ...
            'Date',           date, ...
            'Input_WC',          [wc_folder date filesep 'WC' filesep], ...
...
ecg_bna_cfg.analyses = {'Rpeak_evoked_spike_histogram', 'spike_phase_ECG_cycle'}; % , 
