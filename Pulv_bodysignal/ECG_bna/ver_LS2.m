...
ecg_bna_cfg.spikes_version='ECG_TaskRest';
...
for m=1:numel(monkeys)
    ...
    for s=1:numel(sessions{m})
        ...
        ecg_bna_cfg.session_info(cumulative_sessions) = ...
            ...
            'Input_LFP', {{[ephys_folder 'sites_' monkey '_' date '.mat']}},...  % basically this is where we would change format if we make per site files (instead of per session)
            'Preinj_blocks',  0, ...
