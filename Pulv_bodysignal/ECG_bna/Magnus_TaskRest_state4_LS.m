...
cfg.outNameCap                   = 0;
cfg.spikes_version      = 'ECG_TaskRest_Magnus_merged_state4'; %% this is for loading tuning table (?)
...
monkeys={'Magnus'};
sessions{1}=sort([20220921, 20221115, 20221118, 20221122, 20221206, ...
    ...
    20230614, 20230615, 20230616, 20230621, 20230623]); %20230622, skipped 20221125, - wrong electrode aligniment;
% % 20230531, 20230608 - only cortices (I process them anyway and present separately in the manuscript)
% 
...
cfg.targets_spike_data = {{'VPL', 'dPul', 'MD'}, {'PCC', 'AIP', 'PPC'}};
...
cfg.time.y_lims              = [-30 30];

% phase analysis
cfg.phase.N_phase_bins       = 32;
