...
dates_subject_in{1}             = cellfun(@(x) str2double(x{1}(end-7:end)), filelist.Cornelius_control,'uniformoutput',false);
batching{1}.runs                = cellfun(@(x) x{2}, filelist.Cornelius_control,'uniformoutput',false);  % either empty or spec ific runs specified
group{1}                        = cellfun(@(x) x{1}, filelist.Cornelius_control,'uniformoutput',false);
batching_type{1}                = 3; % 1 run by run, 2 session by session, 3 group by group
batching{1}.range_of_dates      = 0;

subject_ID{2}                   = 'Experimental'; 
dates_subject_in{2}             = cellfun(@(x) str2double(x{1}(end-7:end)), filelist.Cornelius_inactivation,'uniformoutput',false);
batching{2}.runs                = cellfun(@(x) x{2}, filelist.Cornelius_inactivation,'uniformoutput',false);  % either empty or specific runs specified
group{2}                        = cellfun(@(x) x{1}, filelist.Cornelius_inactivation,'uniformoutput',false);
