...
GLO.append_pdfs                     =   0;
GLO.parent_folder                   =   '';
GLO.folder_to_save                  =   'Y:\Projects\Pulv_distractor_spatial_choice\behavior\';
GLO.type_of_free_gaze               =   '6';
GLO.one_subject                     =   0;
GLO.trial_by_trial                  =   0; % for statistics, 0 means calculate statistics bases on average per run
GLO.CDF                             =   1; % 1 plot cumulative distribution function
...
group{1}                        = repmat({'Cornelius'},3,1);
dates_subject_in{1}             = {20190121;20190131;20190213};
batching{1}.runs                = {};  % either empty or spec ific runs specified
batching_type{1}                = 2; % 1 run by run, 2 session by session, 3 group by group
batching{1}.range_of_dates      = 0;

subject_ID{2}='Experimental';
group{2}                        = repmat({'Cornelius'},3,1);
dates_subject_in{2}             = {20190124;20190129;20190201};
