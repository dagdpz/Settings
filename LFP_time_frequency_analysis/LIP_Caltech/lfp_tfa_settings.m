...
lfp_tfa_cfg.monkeys={'HA'}; %,'GU' Gutalin sessions have a different data structure!
   
%% Settings for data folders

% absolute path to the folder where the results of analysis should be stored
lfp_tfa_cfg.results_folder = [base_drive filesep 'Projects\LIP_Caltech\LFP'];
...
if ~lfp_tfa_cfg.process_LFP
    lfp_tfa_cfg.proc_lfp_folder = ...
        [base_drive filesep 'Projects\LIP_Caltech\LFP\paper\Processed LFP'];
end
...
if ~lfp_tfa_cfg.compute_site_average
    lfp_tfa_cfg.analyse_lfp_folder = ...
        [base_drive filesep 'Projects\LIP_Caltech\LFP\paper\LFP Analysis'];
end
...
% lfp_tfa_cfg.info_filepath = 'Y:\Projects\Pulv_oculomotor\ephys\paper\Cur_sorted_neurons.xls';
% lfp_tfa_cfg.info_filepath = 'Y:\Projects\Pulv_oculomotor\ephys\paper\Lin_sorted_neurons.xls';
%%
...
% CREATE MY OWN LOOP
% for sess=1:numel(sitenames)
%     lfp_tfa_cfg.session_info(sess) = ...
%         struct('Monkey',        sitenames{sess}(7:9), ...
%         'Date',           sitenames{sess}(end-11:end-4), ...
%         'Input',         [mainfolder sitenames{sess}], ...
%         'Preinj_blocks',  0, ...
%         'Postinj_blocks', []);
% end

data_folder = 'D:\Projects\LIP_Caltech\converted';
cd 'D:\Projects\LIP_Caltech\converted'
sess_files = dir('*.mat');
sess_names = {sess_files.name};
for sess = 1:numel(sess_names)
    lfp_tfa_cfg.session_info(sess) = ...
        struct('Monkey',        sess_names{sess}(1:2), ...
               'Date',          sess_names{sess}(4:11), ...
               'Input',         [data_folder filesep sess_names{sess}], ...
               'Preinj_blocks', 0, ...
               'Postinj_blocks',[]);
...
%                   time windows
%       'syncsp'      - LFP-LFP phase synchronization spectrum for given 
...
lfp_tfa_cfg.compare.targets = {'LIP_R', 'LIP_L'}; %, 'LIP'
...
if any(strcmp(lfp_tfa_cfg.analyses, 'sync') | strcmp(lfp_tfa_cfg.analyses, 'syncsp'))
    lfp_tfa_cfg.compare.target_pairs = {{'LIP_R', 'LIP_L'}}; 
end
...
% lateral labeling
lfp_tfa_cfg.ref_hemisphere = ''; 

%%
lfp_tfa_cfg.plot_site_average = 0; 
...
lfp_tfa_cfg.analyse_epochs = {%lfp_tfa_states.INI_TRI,        'INI',    -0.4 ,    -0.1  ;...
                              %lfp_tfa_states.FIX_ACQ,        'Facq',    -0.4 ,    -0.1  ;...
                              %lfp_tfa_states.FIX_HOL,        'Fhol',    -0.4 ,    -0.1  ;...
%                               lfp_tfa_states.CUE_ON,         'Fhol',    -0.3 ,    0  ;...
%                               why CUE_ON twice???
                              lfp_tfa_states.CUE_ON,         'Cue' ,    0.07,   0.17 ; ...
%                               lfp_tfa_states.MEM_PER,        'MemE',    0.07,   0.17; ...
                              lfp_tfa_states.TAR_ACQ_INV,    'MemL',    0 ,    0.3  ; ...
                              lfp_tfa_states.SAC_INI,        'PreS',    -0.1, 	-0.01; ...
                              lfp_tfa_states.SAC_END,        'PeriS',   -0.01,  0.05 ; ...
                              lfp_tfa_states.TRI_END,        'TRIe',    0.02,   0.12; ...
%                               lfp_tfa_states.ITI_END,        'ITIe',    0.2,    0.5 ...
                              };
                              %onset and offset times NOT SET YET!!! HOW DO
                              %I CHOOSE? PROBABLY AFTER RUNNING MAIN W/
                              %PLOTS...
...
% 1. lfp_tfa_cfg.trialinfo.ref_tstart = 0.5;
% trial start time = onset time of lfp_tfa_cfg.trialinfo.start_state + 0.5;
lfp_tfa_cfg.trialinfo.ref_tstart = -0; %MAYBE -0.001?
...
lfp_tfa_cfg.compute_avg_across = {'sessions'}; % 
